    ...
    implements KeyEventListener, InteractionHandlerListener
{
    ...
    public static parseParameters(params: URLSearchParams): ParamsStreamScrcpy {
        console.log('[StreamClientScrcpy] parseParameters called with:', params.toString());
        const typedParams = super.parseParameters(params);
        const { action } = typedParams;
        console.log('[StreamClientScrcpy] action:', action, 'expected:', ACTION.STREAM_SCRCPY);
        if (action !== ACTION.STREAM_SCRCPY) {
            console.error('[StreamClientScrcpy] Incorrect action! Got:', action, 'Expected:', ACTION.STREAM_SCRCPY);
            throw Error('Incorrect action');
        }
        const player = Util.parseString(params, 'player', true);
        const udid = Util.parseString(params, 'udid', true);
        const ws = Util.parseString(params, 'ws', true);
        console.log('[StreamClientScrcpy] Parsed params:', { action, player, udid, ws });
        return {
            ...typedParams,
            action,
            player,
