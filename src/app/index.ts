...
window.onload = async function (): Promise<void> {
    ...
    console.log('[ws-scrcpy] Hash:', hash);
    const parsedQuery = new URLSearchParams(hash);
    const action = parsedQuery.get('action');
    console.log('[ws-scrcpy] Action:', action);
    console.log('[ws-scrcpy] All params:', Array.from(parsedQuery.entries()));
    ...
    if (action === StreamClientScrcpy.ACTION && typeof parsedQuery.get('udid') === 'string') {
        console.log('[ws-scrcpy] Starting StreamClientScrcpy...');
